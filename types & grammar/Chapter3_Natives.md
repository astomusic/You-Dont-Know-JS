...
## 3.2 래퍼 박싱하기
원시값에는 프로퍼티나 메서드가 없으으므로 `.length`나 `.toString()`을 접그하려면 객체 래퍼로 감싸줘야 되지만  알아서 박싱(래핑)을 해주므로 다음과 같은 코디가 가능하다.
...
### 3.4.4 Symbol()
심벌(Symbol)은 ES6에서 처음 선보인 새로운 원시 값이다. 심벌은 충돌 염려 없이 객체 프로퍼티로 사용 가능한, 특벽한 '유일 값'이다 (절대적으로 유일함이 보장되지는 않는다!).
